---
title: "Twisted: Improving particle filters by learning modified paths"
subtitle: ""
author: "Joshua J Bon"
...
- Mutation: $m_p$ is the density for $M_p$
- Potential: $G_p$ is the likelihood of $y_p$
---

## Particle filters

Initial distribution $M_0$ (or $m_0$) then for $p = 1, 2, \ldots, n$

--

.full-width[.content-box-red[
`r emo::ji("pepper")` **Update**
- Weight with potential $G_{p-1}$
...
`r emo::ji("pepper")` **Predict**
- Mutate samples with $M_p$
]]

--

Finally, terminal update step with $G_n$ 
...
$$\text{For}~S \in \mathcal{X}, \quad \mu(S) = \int_{S} \color{red}{p_\mu(x)\text{d}x}$$

$$\text{For}~\varphi:\mathsf{X}\rightarrow \mathbb{R}, \quad \mu(\varphi) = \int_{\mathsf{X}} \varphi(x) \color{red}{p_\mu(x)\text{d}x}$$
]


<br>
- Implicit integrals/measures are defined by $\varphi(x) \mu(\text{d}x) \Longleftrightarrow \int_{S} \varphi(x) \color{red}{p_\mu(x)\text{d}x}$
...
$$\text{For}~v \in \mathsf{X},\varphi:\mathsf{X}\rightarrow \mathbb{R}, \quad K(\varphi)(v) = \int_{\mathsf{X}} \varphi(x) K(v,\text{d}x)$$
]
...
- $G_0$ potential, weights initial **distribution**
 - $\hat\gamma_0(\text{d}x) = \eta_0(\text{d}x)G_0(x)$
 - $\hat\eta_0(\text{d}x) = \hat\gamma_0(\text{d}x) /Z_1$
- $M_1$ kernel, mutates updated **distribution**
 - $\eta_1(S) = \int_{\mathsf{X}} \eta_0(\text{d}x)M_1(x, S)$
...
- $G_0$ potential, weights initial **particles**
 - Estimate $Z_1$, self-normalised IS
- $M_1$ kernel, mutates updated **particles**
...
With $\gamma_0 = M_0$ we can define marginal predictive via recursion 

$$\begin{aligned}\gamma_{p}(S) &= \int_{\mathsf{X}} \gamma_{p-1}(\text{d}x_{p-1})\color{red}{G_{p-1}(x_{p-1})}\color{blue}{M_{p}(x_{p-1},S)}\\
\eta_{p}(S) &= \frac{\gamma_{p}(S)}{Z_p}\end{aligned}$$

--

And updated versions

$$\begin{aligned}\hat{\gamma}_{p}(\text{d}x_p) &= \gamma_{p}(\text{d}x_p)\color{red}{G_p(x_p)} \\ 
~\\
\hat{\eta}_{p}(S) &= \frac{\hat{\gamma}_{p}(S)}{\hat{Z}_p}\end{aligned}$$
...
Changes intermediate distributions, $\eta^{\psi}_p \neq \eta_p$
...
`r emo::ji("exclamation_mark")` Almost all use-cases involve Gaussian mutation kernels and twisting functions
